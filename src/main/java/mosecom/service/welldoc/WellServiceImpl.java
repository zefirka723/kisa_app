...
//    @Override
//    @Transactional
//    public Well save(WellFullProjection dto, MultipartFile[] files, int docType) throws IllegalStateException, IOException, ParseException {
//        Well well;
...
//    }
    ...
    // НОВОЕ СОХРАНЕНИЕ
    @Transactional
    public Well save(WellFullProjection dto, MultipartFile[] files, DocTypes docType) throws IllegalStateException, IOException, ParseException {
        Well well;
        if (dto.getId() != null && dto.getId() > 0) {
            well = wellRepository.getOne(dto.getId());
        } else {
